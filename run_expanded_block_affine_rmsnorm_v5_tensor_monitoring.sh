...
PREFIX_CONFIG="configs/vit_tiny_tinyimagenet_rmsnorm_v5.yaml"
TARGET_CONFIG="configs/vit_tiny_tinyimagenet_expanded_block_affine_rmsnorm_v5_tensor_monitoring.yaml"
TARGET_OUTPUT="$PROJECT_ROOT/outputs/expanded_block_affine_rmsnorm_v5_tensor_monitoring"
TARGET_PREFIX_DIR="$TARGET_OUTPUT/prefix"
TARGET_PREFIX="$TARGET_PREFIX_DIR/checkpoint_step_002500.pt"
SOURCE_PREFIX="${SOURCE_PREFIX:-$TARGET_PREFIX}"
PREFLIGHT_JSON="$TARGET_OUTPUT/expanded_block_affine_rmsnorm_v5_preflight.json"
...
# RMSNorm changes the architecture, so V5 creates its own step-2500 prefix.
# A supplied prefix is accepted only after the RMSNorm preflight below.
if [[ "$SOURCE_PREFIX" == "$TARGET_PREFIX" && ! -f "$TARGET_PREFIX" ]]; then
...
if [[ ! -f "$SOURCE_PREFIX" ]]; then
  echo "V5 RMSNorm shared prefix was not created: $SOURCE_PREFIX" >&2
  exit 1
fi
...
"$PYTHON" scripts/expanded_block_affine_rmsnorm_v5_preflight.py \
  --checkpoint "$TARGET_PREFIX" \
  --expected-sha256 "$TARGET_SHA" \
  > "$PREFLIGHT_JSON"
cat "$PREFLIGHT_JSON"

"$PYTHON" run_pipeline_expanded_block_affine_rmsnorm_v5.py \
