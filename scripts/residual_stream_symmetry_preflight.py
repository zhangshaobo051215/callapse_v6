...
def main() -> None:
    ...
    if source_policy != "hidden_matrices":
        raise ValueError(
            f"unexpected shared-prefix policy: {source_policy}")
    ...
    expected_additions, expected_removals = expected_policy_delta(model)

    if set(controlled) != residual_stream_parameter_names(model):
        raise ValueError("controlled scope differs from residual-stream audit")
    if len(controlled) != 52 or len(uncontrolled) != 100:
        raise ValueError(
            f"expected controlled/uncontrolled=52/100, got "
            f"{len(controlled)}/{len(uncontrolled)}")
    if len(units) != 52:
        raise ValueError(
            f"expected 52 independent control units, got {len(units)}")
    ...
    if len(additions) != 28 or len(removals) != 24:
        raise ValueError(
            f"expected additions/removals=28/24, got "
            f"{len(additions)}/{len(removals)}")
    forbidden = {
        name for name in controlled
        if (
            ".attn.qkv." in name
            or ".mlp.fc1." in name
            or "norm" in name
    ...
    if forbidden:
        raise ValueError(
            f"forbidden parameters are controlled: {sorted(forbidden)}")
    ...
    if invalid_references:
        raise ValueError(
            f"invalid reference norms: {invalid_references}")
    ...
    if unequal_states:
        raise ValueError(
            f"optimizer state values changed during migration: "
            f"{unequal_states}")
